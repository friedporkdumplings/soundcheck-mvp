...
export default function PracticePage() {
  ...
  const [wrongSeatNotice, setWrongSeatNotice] = useState(false);
  ...
  function chooseRushSeat(seat: string) {
    if (unavailable.has(seat) || seat !== target) {
      setWrongSeatNotice(true);
      window.setTimeout(() => setWrongSeatNotice(false), 500);
      return;
    }
  ...
  return <main className="min-h-screen bg-white px-5 py-6 text-[#183153] sm:px-10 sm:py-10"><div className="mx-auto max-w-6xl">
    ...
    {screen === "rush" && <><GameFrame eyebrow={`Seat Rush · ${rushSpeed[difficulty].label}`} title={`Your target: ${target}`} detail="Find the seat before inventory disappears. Your target is protected for the first three seconds."><SeatGrid unavailable={unavailable} selected={selectedSeat} onSeat={chooseRushSeat} /></GameFrame>{wrongSeatNotice && <div className="pointer-events-none fixed inset-x-0 top-20 z-50 mx-auto w-fit border border-[#193a68] bg-[#f6b6cf] px-3 py-1.5 text-xs font-black text-[#183153] shadow-[2px_2px_0_#193a68]">This seat is already taken!</div>}</>}
