...
const adaptiveSeatSize = "clamp(14px, min(1.45vw, 2.35vh), 28px)";
const adaptiveLabelSize = "clamp(18px, min(1.6vw, 2.55vh), 28px)";
...
export default function PracticePage() {
  ...
  useEffect(() => {
    ...
    const countdown = window.setInterval(() => setWaitingMinutes((minutes) => Math.max(0, minutes - 2)), 700);
    const timer = window.setTimeout(() => setScreen("queue"), 2800);
  ...
  useEffect(() => {
    ...
    const finalCount = window.setTimeout(() => setQueueNumber(1), 3600);
    const finish = window.setTimeout(() => { setRushStartedAt(Date.now()); setScreen("rush"); }, 4400);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setSniperCountdown((count) => count - 1), 700);
    const go = window.setTimeout(() => setScreen("sniper"), 2200);
    return () => { window.clearInterval(timer); window.clearTimeout(go); };
  }, [screen]);

  useEffect(() => {
    if (screen === "rush" || screen === "sniper") window.scrollTo({ top: 0, behavior: "smooth" });
  }, [screen]);
  ...
  return <main className="min-h-screen bg-white px-5 py-6 text-[#183153] sm:px-10 sm:py-10"><div className="mx-auto max-w-6xl">
    <header className="sticky top-0 z-40 flex items-center justify-between border-b-2 border-[#193a68] bg-white py-5"><Link href="/" className="flex items-center gap-3"><span className="h-3 w-3 rounded-full border border-[#193a68] bg-[#8ed1ff]" /><span className="text-sm font-black tracking-[0.22em] uppercase">Soundcheck</span></Link><nav className="flex items-center gap-4"><Link href="/practice" className="text-xs font-bold tracking-[0.12em] text-[#31577f] uppercase underline decoration-2 underline-offset-4">Ticketing practice</Link><Link href="/#event-library" className="text-xs font-bold tracking-[0.12em] text-[#31577f] uppercase underline decoration-2 underline-offset-4">Event library</Link><Link href="/" className="text-xs font-bold tracking-[0.12em] text-[#52739a] uppercase">Your concert brief</Link></nav></header>
...
function ModeSelect({ onRush, onSniper, bestRush, bestSniper }: { onRush: () => void; onSniper: () => void; bestRush: number | null; bestSniper: number | null }) { return <section className="py-16 sm:py-20"><p className="text-sm font-bold tracking-[0.16em] text-[#52739a] uppercase">Soundcheck training</p><h1 className="mt-4 text-5xl font-black tracking-[-0.055em] sm:text-7xl">Ticketing Practice</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-[#31577f]">Build calm, fast habits before a high-demand sale. These are practice simulations only—no real tickets are sold.</p><div className="mt-8 flex flex-wrap gap-2 border-2 border-[#193a68] bg-white p-4 text-sm font-bold text-[#31577f] shadow-[3px_3px_0_#193a68]"><span className="text-[#183153]">What you&apos;ll train:</span><span>rapid seat scanning</span><span aria-hidden="true">·</span><span>quick target clicks</span><span aria-hidden="true">·</span><span>queue pressure</span></div><div className="mt-8 grid gap-5 md:grid-cols-2"><ModeCard title="Seat Rush" detail="Move through a simulated queue, then find your assigned seat before inventory disappears." score={bestRush === null ? "No best time yet" : `Best find: ${bestRush.toFixed(1)}s`} button="Play Seat Rush" onClick={onRush} /><ModeCard title="Seat Sniper" detail="Twenty seconds of rapid target-hunting to sharpen seat scanning and quick clicks." score={bestSniper === null ? "No high score yet" : `High score: ${bestSniper}`} button="Play Seat Sniper" onClick={onSniper} /></div></section>; }
...
function GameFrame({ eyebrow, title, detail, children }: { eyebrow: string; title: string; detail: string; children: ReactNode }) { return <section className="mt-5 mb-[max(3rem,6vh)]"><div className="border-2 border-[#193a68] bg-[#ccecff] p-4 shadow-[4px_4px_0_#193a68]"><p className="text-xs font-black tracking-[0.14em] text-[#52739a] uppercase">{eyebrow}</p><h1 className="mt-2 text-2xl font-black sm:text-4xl">{title}</h1><p className="mt-2 text-sm text-[#31577f]">{detail}</p></div><div className="mt-4 overflow-x-auto border-2 border-[#193a68] bg-white p-3 shadow-[4px_4px_0_#193a68]">{children}</div></section>; }
function SeatGrid({ unavailable = new Set<string>(), target, selected, notice, onSeat }: { unavailable?: Set<string>; target?: string; selected?: string; notice?: string | null; onSeat: (seat: string) => void }) { return <div className="relative mx-auto w-fit"><div className="grid items-center gap-0.5" style={{ gridTemplateColumns: `${adaptiveLabelSize} repeat(26, ${adaptiveSeatSize})` }}><span />{letters.map((letter) => <span key={letter} className="text-center text-[9px] font-black text-[#52739a]">{letter}</span>)}{Array.from({ length: 26 }, (_, row) => <div key={row} className="contents"><span className="text-center text-[9px] font-black text-[#52739a]">{row + 1}</span>{letters.map((letter) => { const seat = `${letter}${row + 1}`; const gone = unavailable.has(seat); const activeTarget = target === seat; const picked = selected === seat; return <button key={seat} aria-disabled={gone} onClick={() => onSeat(seat)} aria-label={`Seat ${seat}`} style={{ height: adaptiveSeatSize, width: adaptiveSeatSize }} className={`rounded-full border border-[#193a68] transition ${gone ? "cursor-not-allowed bg-[#193a68]" : picked ? "bg-[#f6b6cf]" : activeTarget ? "bg-[#193a68]" : "bg-[#eef8ff] hover:bg-[#8ed1ff]"}`} />; })}</div>)}</div>{notice && <div className="pointer-events-none absolute left-1/2 top-1/2 z-10 -translate-x-1/2 -translate-y-1/2 border border-[#193a68] bg-[#f6b6cf]/75 px-3 py-1.5 text-xs font-black text-[#183153] shadow-[2px_2px_0_#193a68]">{notice}</div>}</div>; }
