...
const rushSpeed: Record<Difficulty, { interval: number; seats: [number, number]; label: string }> = {
  ...
  hard: { interval: 320, seats: [14, 24], label: "Hard" },
};
...
export default function PracticePage() {
  ...
  return <main className="min-h-screen bg-white px-5 py-6 text-[#183153] sm:px-10 sm:py-10"><div className="mx-auto max-w-6xl">
    ...
    {screen === "rush" && <GameFrame eyebrow={`Seat Rush · ${rushSpeed[difficulty].label}`} title={`Your target: ${target}`} detail="Find the seat before inventory disappears. Your target is protected for the first three seconds."><SeatGrid unavailable={unavailable} selected={selectedSeat} onSeat={chooseRushSeat} /></GameFrame>}
...
function SeatGrid({ unavailable = new Set<string>(), target, selected, onSeat }: { unavailable?: Set<string>; target?: string; selected?: string; onSeat: (seat: string) => void }) { return <div className="min-w-[720px]"><div className="grid items-center gap-1" style={{ gridTemplateColumns: "28px repeat(26, 24px)" }}><span />{letters.map((letter) => <span key={letter} className="text-center text-[10px] font-black text-[#52739a]">{letter}</span>)}{Array.from({ length: 26 }, (_, row) => <div key={row} className="contents"><span className="text-center text-[10px] font-black text-[#52739a]">{row + 1}</span>{letters.map((letter) => { const seat = `${letter}${row + 1}`; const gone = unavailable.has(seat); const activeTarget = target === seat; const picked = selected === seat; return <button key={seat} disabled={gone} onClick={() => onSeat(seat)} aria-label={`Seat ${seat}`} className={`h-6 w-6 rounded-full border border-[#193a68] transition ${gone ? "cursor-not-allowed bg-[#193a68]" : picked ? "bg-[#f6b6cf]" : activeTarget ? "bg-[#193a68]" : "bg-[#eef8ff] hover:bg-[#8ed1ff]"}`} />; })}</div>)}</div></div>; }
