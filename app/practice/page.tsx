...
function GameFrame({ eyebrow, title, detail, children }: { eyebrow: string; title: string; detail: string; children: ReactNode }) { return <section className="mt-5"><div className="border-2 border-[#193a68] bg-[#ccecff] p-4 shadow-[4px_4px_0_#193a68]"><p className="text-xs font-black tracking-[0.14em] text-[#52739a] uppercase">{eyebrow}</p><h1 className="mt-2 text-2xl font-black sm:text-4xl">{title}</h1><p className="mt-2 text-sm text-[#31577f]">{detail}</p></div><div className="mt-4 overflow-x-auto border-2 border-[#193a68] bg-white p-3 shadow-[4px_4px_0_#193a68]">{children}</div></section>; }
function SeatGrid({ unavailable = new Set<string>(), target, selected, notice, onSeat }: { unavailable?: Set<string>; target?: string; selected?: string; notice?: string | null; onSeat: (seat: string) => void }) { return <div className="relative min-w-[560px]"><div className="grid items-center gap-0.5" style={{ gridTemplateColumns: "22px repeat(26, 18px)" }}><span />{letters.map((letter) => <span key={letter} className="text-center text-[9px] font-black text-[#52739a]">{letter}</span>)}{Array.from({ length: 26 }, (_, row) => <div key={row} className="contents"><span className="text-center text-[9px] font-black text-[#52739a]">{row + 1}</span>{letters.map((letter) => { const seat = `${letter}${row + 1}`; const gone = unavailable.has(seat); const activeTarget = target === seat; const picked = selected === seat; return <button key={seat} aria-disabled={gone} onClick={() => onSeat(seat)} aria-label={`Seat ${seat}`} className={`h-[18px] w-[18px] rounded-full border border-[#193a68] transition ${gone ? "cursor-not-allowed bg-[#193a68]" : picked ? "bg-[#f6b6cf]" : activeTarget ? "bg-[#193a68]" : "bg-[#eef8ff] hover:bg-[#8ed1ff]"}`} />; })}</div>)}</div>{notice && <div className="pointer-events-none absolute left-1/2 top-1/2 z-10 -translate-x-1/2 -translate-y-1/2 border border-[#193a68] bg-[#f6b6cf]/75 px-3 py-1.5 text-xs font-black text-[#183153] shadow-[2px_2px_0_#193a68]">{notice}</div>}</div>; }
