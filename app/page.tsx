...
export default function Home() {
  ...
  const { events: historyEvents, hydrated: historyHydrated, find: findHistoryEvent, remember: rememberEvent } = useEventHistory();
  ...
  return (
    <main className="min-h-screen bg-white px-5 py-6 text-[#183153] sm:px-10 sm:py-10">
      <div className="mx-auto max-w-6xl">
        <header className="sticky top-0 z-40 flex items-center justify-between border-b-2 border-[#193a68] bg-white py-5">
          ...
          <nav className="flex items-center gap-4"><a href="/practice" className="text-xs font-bold tracking-[0.12em] text-[#31577f] uppercase">Ticketing practice</a><a href="#event-library" onClick={(navigationEvent) => { navigationEvent.preventDefault(); setLibraryOpen(true); }} className="text-xs font-bold tracking-[0.12em] text-[#31577f] uppercase">Event library</a><span className="text-xs font-bold tracking-[0.12em] text-[#52739a] uppercase underline decoration-2 underline-offset-4">Your concert brief</span></nav>
        </header>

        <section className="grid gap-8 py-14 sm:py-20 lg:grid-cols-[1.15fr_.85fr] lg:items-end"><div><p className="mb-4 text-sm font-bold tracking-[0.16em] text-[#52739a] uppercase">Get ready for the show</p><h1 className="max-w-4xl text-5xl font-black tracking-[-0.055em] text-[#183153] sm:text-7xl">Less searching.<br />More showing up.</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-[#31577f]">Paste one exact Ticketmaster event page. Soundcheck turns it into a practical brief with show timing, official venue rules, weather, routes, seat views, and recent tour context. Then use Ticketing Practice to sharpen your seat-finding speed before a high-demand sale.</p><div className="mt-7 flex flex-wrap gap-x-5 gap-y-2 text-xs font-black tracking-[0.1em] text-[#31577f] uppercase"><span className="border-l-2 border-[#5baeea] pl-3">Event brief</span><span className="border-l-2 border-[#5baeea] pl-3">Venue intel</span><span className="border-l-2 border-[#5baeea] pl-3">Ticketing practice</span></div></div><aside className="border-2 border-[#193a68] bg-[#eef8ff] p-5 shadow-[4px_4px_0_#193a68]"><p className="text-xs font-black tracking-[0.14em] text-[#52739a] uppercase">What you&apos;ll get</p><div className="mt-4"><div className="border-b border-[#9db9d6] py-3 first:pt-0"><p className="text-sm font-black">The essentials</p><p className="mt-1 text-xs leading-5 text-[#52739a]">Date, time, venue, and the official event page.</p></div><div className="border-b border-[#9db9d6] py-3"><p className="text-sm font-black">The practical stuff</p><p className="mt-1 text-xs leading-5 text-[#52739a]">Entry rules, weather, transit, parking, and seat views.</p></div><div className="pt-3"><p className="text-sm font-black">Less repeat searching</p><p className="mt-1 text-xs leading-5 text-[#52739a]">Recent briefs stay ready in History for 48 hours on this device.</p></div></div></aside></section>
        ...
        {event && !loading && !demoLoading && <motion.section className="fixed inset-0 z-50 overflow-y-auto bg-white px-5 py-6 text-[#183153] sm:px-10 sm:py-10" variants={containerVariants} initial="hidden" animate="visible" viewport={{ once: true }}><div className="mx-auto max-w-6xl"><OverlayHeader active="brief" onOpenLibrary={() => setLibraryOpen(true)} /><div className="mt-6 mb-6 flex items-center justify-between border-b-2 border-[#193a68] pb-4"><div><p className="text-xs font-black tracking-[0.14em] text-[#52739a] uppercase">Your concert brief</p><p className="mt-1 text-sm text-[#31577f]">Everything for this show, in one focused view.</p></div><button onClick={() => setEvent(null)} className="border-2 border-[#193a68] bg-white px-4 py-3 text-sm font-black text-[#183153] shadow-[3px_3px_0_#193a68] transition hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[1px_1px_0_#193a68]">← Back to dashboard</button></div>
        ...
        {libraryOpen && <section className="fixed inset-0 z-[60] overflow-y-auto bg-white px-5 py-6 text-[#183153] sm:px-10 sm:py-10" aria-label="Your event library"><div className="mx-auto max-w-6xl"><OverlayHeader active="library" onOpenLibrary={() => setLibraryOpen(true)} /><div className="mt-6 mb-6 flex items-center justify-between border-b-2 border-[#193a68] pb-4"><div><p className="text-xs font-black tracking-[0.14em] text-[#52739a] uppercase">Your event library</p><p className="mt-1 text-sm text-[#31577f]">Reopen recent briefs or keep the shows you care about in one place on this device.</p></div><button onClick={() => setLibraryOpen(false)} className="border-2 border-[#193a68] bg-white px-4 py-3 text-sm font-black text-[#183153] shadow-[3px_3px_0_#193a68] transition hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[1px_1px_0_#193a68]">← Back to dashboard</button></div><div className="border-2 border-[#193a68] bg-white p-5 shadow-[4px_4px_0_#193a68]"><div className="grid gap-5 lg:grid-cols-2"><section className="border-t-2 border-[#193a68] pt-4"><p className="text-xs font-black tracking-[0.14em] text-[#52739a] uppercase">Previously viewed</p><p className="mt-2 text-sm leading-6 text-[#52739a]">Briefs stay ready for 48 hours and reopen before Soundcheck reaches Ticketmaster or a public event page again.</p>{historyHydrated && historyEvents.length > 0 ? <div className="mt-4 grid gap-3">{historyEvents.map((item) => <button key={item.sourceUrl} onClick={() => loadSavedEvent(item)} className="border border-[#193a68] bg-[#eef8ff] p-4 text-left shadow-[3px_3px_0_#193a68] transition hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-[2px_2px_0_#193a68]"><p className="font-black text-[#183153]">{item.name}</p><p className="mt-1 text-sm text-[#52739a]">{item.venue} · {item.date}</p></button>)}</div> : <p className="mt-4 text-sm text-[#52739a]">Briefs you load will appear here for the next 48 hours.</p>}</section><section className="border-t-2 border-[#193a68] pt-4"><p className="text-xs font-black tracking-[0.14em] text-[#52739a] uppercase">Saved events</p><p className="mt-2 text-sm leading-6 text-[#52739a]">Keep Ticketmaster links for the shows you care about, then reopen them instantly when a device copy is available.</p>{savedEventsHydrated && savedEvents.length > 0 ? <div className="mt-4 grid gap-3">{savedEvents.map((item) => <button key={item.sourceUrl} onClick={() => loadSavedEvent(item)} className="border border-[#193a68] bg-white p-4 text-left shadow-[3px_3px_0_#193a68] transition hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-[2px_2px_0_#193a68]"><p className="font-black text-[#183153]">{item.name}</p><p className="mt-1 text-sm text-[#52739a]">{item.venue} · {item.date}</p></button>)}</div> : <p className="mt-4 text-sm text-[#52739a]">Load a brief, then use Save event to keep its Ticketmaster link here.</p>}</section></div></div></div></section>}
      </div>
    </main>
  );
}
...
function OverlayHeader({ active, onOpenLibrary }: { active: "library" | "brief"; onOpenLibrary: () => void }) { return <header className="sticky top-0 z-40 flex items-center justify-between border-b-2 border-[#193a68] bg-white py-5"><div className="flex items-center gap-3"><span className="h-3 w-3 rounded-full border border-[#193a68] bg-[#8ed1ff]" /><span className="text-sm font-black tracking-[0.22em] text-[#183153] uppercase">Soundcheck</span></div><nav className="flex items-center gap-4"><a href="/practice" className="text-xs font-bold tracking-[0.12em] text-[#31577f] uppercase">Ticketing practice</a><a href="#event-library" onClick={(navigationEvent) => { navigationEvent.preventDefault(); onOpenLibrary(); }} className={`text-xs font-bold tracking-[0.12em] text-[#31577f] uppercase ${active === "library" ? "underline decoration-2 underline-offset-4" : ""}`}>Event library</a><span className={`text-xs font-bold tracking-[0.12em] text-[#52739a] uppercase ${active === "brief" ? "underline decoration-2 underline-offset-4" : ""}`}>Your concert brief</span></nav></header>; }
